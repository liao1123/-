...
class Label:
    """Class for Label"""
    __slots__ = ['lenV', 'blocksize', 'K1', 'K2', 'K3', 'K4']

    def __init__(self, lenV: int = 1, blocksize: int = 16):
        self.lenV = lenV
    ...
    def Enc(self, K, L):
        ...
        return {PF(w, cipher2, self.blocksize):
                    faster_xor(b''.join([G(i, cipher4, self.blocksize) for i in L[w]]),
    ...
    def EncData(self, M):
        ...
        m, v = M
        return {G(key, cipher4, self.blocksize): m[key] for key in tqdm(m.keys(), desc="Encrypting")}



class User:
    __slots__ = ['blocksize', 'lenV', 'hashLen', 'hashLenByte', 'ss']
    """
    ...
    def __init__(self, blocksize: int, lenV: int, hashLen: int, K: SSECMRKey, ss: int):
        """
